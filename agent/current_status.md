...
3. **ADR-009 (Generic ObjectType) + audit ticketów GitHub** (2026-04-27, PR #127 squash-merged jako `ce590a6`, audit issues PR #129 squash-merged). Pełen ADR-009 w `01-architektura-pim.md` §13 — generic `ObjectType` z predefiniowanymi Product/Category/Asset jako `is_built_in=true` instancje + custom kindy (`Customer`, `Supplier`, `PriceList`) odblokowane w Fazie 2/3. Wpływ: epik 0.3 reestymowany 16-20h → **36-50h** (+16-25h finansowane ze zwolnionego budżetu epiku 0.7 przeniesionego do Fazy 2). Faza 0 total: **170-235h pełny / 156-216h okrojony** (saldo netto -31 do -39h vs poprzedni budżet 201-274h: rewizja 2026-04-27 zwolniła 51-69h przez przeniesienie epików 0.7/0.8/0.9, ADR-009 dodał 20-30h w epiku 0.3 — single source of truth: sumy epików §3.3 + milestone tabela §3.4 planu). Pojęcie „Family" deprecated. Sugar paths `/api/products`, `/api/categories`, `/api/assets` zostają user-facing (lepszy DX integratorów). Audit 91 issues: epik 0.3 major rebody (#31-#40 + nowy 0.3.11 / GH #128), epiki 0.4/0.5/0.6/0.10/0.11 light rebody, agent (Faza 2) + integracje (Faza 1) minimalna generalizacja. Plus nowe ryzyko **R-29: Over-engineering generic ObjectType w MVP** (mitigacja: feature flag, predefined UX, benchmark `attributes_indexed` na 10k×200×3 kindach).
> **Wcześniej:** Ticket #14 (0.0.14) zamknięty (squash-merged do main 2026-04-27 jako `10c6de2`). Perf profile k6 na `GET /api/products?page=1` — 10 VUs p95=105 ms, single-user 18.7 ms.

## Bieżący stan
Sprint 0 = 12/13 ticketów done (#1 setup, #2 multi-tenancy, #3 ApiResource Product, #4 LexikJWT, #5 admin Refine, #10 Playwright E2E, #11 quality gates, #12 isolation smoke, #13 memory benchmark, #14 perf profile, #15 pgBackRest stub, #16 audit + findings). Pozostałe: #9 (manual demo + gate decision).
...
| # | Ticket | Komentarz |
|---|---|---|
| #9 (0.0.9) | Manualny E2E + screencast (gate decision) | Ostatni Sprint-0 ticket. Verdict GREEN/RED — predykcja **GREEN** (12/13 zielone, brak open blockerów). |

## Postęp po fazach (po rewizji zakresu)
- [ ] Sprint 0 (gate decision) — **12/13 ticketów done** — issues #1-#5, #9-#16 (#6, #7, #8 przeniesione do Faza 1/2)
...
- [x] **#15 / 0.0.15** — pgBackRest + WAL stub (PR pending — custom postgres image z pgbackrest+cron, MinIO repo via Caddy TLS terminator, restore test 1005→1008 markery odzyskane)
