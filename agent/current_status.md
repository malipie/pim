...
## Sub-faza: MVP-ALPHA — Sprint 0 ZAMKNIĘTY ✅ 13/13, gate decision GREEN (2026-04-28). Następny: epik 0.1 (#17-#23).

## Ostatnie 3 akcje
1. **Sprint 0 ZAMKNIĘTY — gate decision GREEN** (2026-04-28). Ticket #9 (manual E2E + screencast) wykonany manualnie przez operatora — auth + tenant isolation + product CRUD działają na obu tenantach (`demo` + `acme`). Milestone "Sprint 0 — Vertical Slice" (GH #1) zamknięty: 13/13 issues closed. PR #130 (`feat/sprint-0-pgbackrest`) squash-merged do main jako `868b87c` (pgBackRest + WAL stub + post-audit ADR-009 fixes). Kandydaci do Definicji "Done" Sprint-0 spełnieni: PHPStan max + Biome strict + PHPUnit (TenantAssignmentListenerTest 4 + ProductApiTest 6 + TenantIsolationTest 4 + AuthApiTest 5) + ApiTestCase + Playwright E2E (9 cases — 5 auth + 4 CRUD) + composer/pnpm audit + manual smoke + perf profile (10 VUs p95 = 105 ms) + memory benchmark (50k = 14 MiB FLAT) + restore test (1005→1008 markery odzyskane). Świadome odejścia z Sprintu 0 spisane w `Project Plan/06-sprint-0-findings.md` + `agent/lessons.md`. **Następny krok: MVP-Alpha epik 0.1 (Infrastructure i fundamenty, #17-#23)**.
...
## Bieżący stan
Sprint 0 = **13/13 ZAMKNIĘTE** (gate GREEN 2026-04-28). Wszystkie issues #1-#5, #9-#16 closed. Milestone GH #1 closed.

**MVP-Alpha rozpoczęte.** Pierwszy ticket: **#17 (0.1.1) — Bootstrap bundle struktura DDD** (epik 0.1 Infrastructure i fundamenty, #17-#23). Patrz `Project Plan/02-plan-projektu-pim.md` §3.3 epik 0.1.
...
| # | Ticket | Komentarz |
|---|---|---|
| #17 (0.1.1) | Bootstrap bundle struktura DDD | Pierwszy ticket epiku 0.1 (Infrastructure i fundamenty). Bounded contexts `Catalog`/`Channel`/`Asset`/`Integration`/`Identity`/`Agent`/`ApiConfigurator` jako osobne bundle'e w `apps/api/src/`. |
| #18-#23 (0.1.2-0.1.7) | Pozostałe ticketów epiku 0.1 | Symfony Messenger, Mercure, MinIO Flysystem, observability skeleton. Patrz `Project Plan/02-plan-projektu-pim.md` §3.3. |
