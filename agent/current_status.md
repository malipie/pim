...
## Sub-faza: MVP-ALPHA — epik 0.2 IN PROGRESS (#24 done, 6 do zrobienia). Branch: `feat/0.2.1-rbac-schema`.

## Ostatnie 3 akcje
1. **#24 (0.2.1) RBAC schema baseline** (2026-04-28, branch `feat/0.2.1-rbac-schema`). Pierwszy ticket epiku 0.2. **Nowe encje:** `Role` (id, tenant_id NULLABLE = global, code, name) z M2M do `Permission` (id, code, resource, action) przez `role_permissions`; junction `user_roles` dla User↔Role. Unique constraints: `roles(tenant_id, code)`, `permissions(resource, action)` + `permissions(code)`. **Rozszerzenia:** `User` dostał `status VARCHAR(16) DEFAULT 'active'` + `last_login_at TIMESTAMP NULL` + relację M2M `$assignedRoles` (obok legacy `roles JSON` zostającego do cleanup w #27); `Tenant` dostał `domain VARCHAR(255) NULL UNIQUE` + `plan VARCHAR(32) DEFAULT 'starter'`. **Migracja:** `Version20260428131449` (22 SQL queries, round-trip up/down/up OK). **Repos:** `RoleRepository::findGlobalByCode/findByCode`, `PermissionRepository::findByResourceAction/findByCode`. **Tests:** `RbacSchemaTest` (5 cases — global vs tenant role, permission unique, M2M roundtrip, status+last_login_at roundtrip, tenant domain+plan roundtrip) — real Postgres przez `ApiTestCase` + Foundry `ResetDatabase`. **Quality gates:** PHPStan max + cs-fixer + PHPUnit 27/27 + Playwright 9/9 + migration round-trip. **Świadome odejścia (do udokumentowania w #27 cleanup):** (a) `User.roles JSON` zostaje obok M2M — Sprint-0 LexikJWT nadal czyta JSON, refactor `getRoles()` mergujący M2M w #25/#27; (b) `Tenant.plan` jako VARCHAR (nie enum DB-level) — tier'y się zmieniają, walidacja w aplikacji; (c) `Role.tenant_id NULL=global` pozwala seederowi #27 utworzyć 4 standardowe role jako global. **Blocker przy Playwright:** `pim-caddy-1` był w stanie `Created` po `pnpm stack:up` — wymagał ręcznego `docker compose start caddy`. Możliwe że stack:up trafia w warunki wyścigu (caddy zależy od api healthy, api ma długi startup w worker-mode). Jeśli się powtórzy → osobny ticket diagnostyczny.
2. **Epik 0.1 ZAMKNIĘTY — Infrastructure i fundamenty** (2026-04-28). 7/7 ticketów (#17-#23) closed. **Audit recon ujawnił że 4 z 7 było faktycznie zrobione w Sprincie 0** (#18 docker-compose pełna forma, #21 GitHub Actions CI 3 workflows, #22 husky + lint-staged + commitlint, #23 baseline migrations) — zamknięte komentarzami audytu z linkami do Sprint-0 PR-ów. **Nowa praca w jednym PR (`feat/epic-0.1-foundations`):** (a) **#17:** CONTRIBUTING.md (Conventional Commits, branch naming, DoD, hook expectations) + LICENSE (UNLICENSED proprietary) + README refresh (stack components table, Backup&Restore section, Sprint 0 status); (b) **#19:** scaffolding 5 brakujących bounded contexts (`Channel`, `Asset`, `Integration`, `Agent`, `ApiConfigurator`) z 4-warstwowym DDD layout (`Domain/`, `Application/`, `Infrastructure/`, `Presentation/`) + `README.md` per kontekst tłumaczący zakres + który epik dorobi implementację; Catalog + Identity (już istniejące ze Sprintu 0) doequalizowane do 4-warstwowego layoutu; (c) **#20:** 5 placeholder pages w admin (`/attributes`, `/object-types`, `/categories`, `/assets`, `/channels`) używających `<ComingSoon resource epic issue />` komponentu (link do GH issue + i18n PL/EN), sidebar rozszerzony z 6 navigation entries + "Wkrótce/Soon" badge dla niezimplementowanych; (d) **#23:** `pim:db:reset` CLI command w `apps/api/src/Maintenance/DatabaseResetCommand.php` — drop+create+migrate (+optional fixtures) w jednym shocie z guards na APP_ENV=prod (`--force-prod` required). **Per-context migrations dirs odrzucone jako over-engineering** (single Postgres, Symfony default OK). **Następny krok:** epik 0.2 (Identity & Access, #24-#30) — RBAC, organizacje/zespoły, scheb/2fa-bundle, password reset.
