...
### ETAP 2 — w toku
  ...
  - [→] **IMP2-2.3 (#1479)** — realna pauza/wznowienie/anulowanie + checkpoint crash-resilient. NEXT (architektonicznie głęboki: status-check per chunk, checkpoint_offset/phase, resume idempotent, lock refresh, state machine, koordynacja z 1.8 two-pass + 2.4 undo).
  - [ ] Etap 2 reszta (#1480–1486): undo-log, RLS GUC, perf/bench, security plików, równoległość, backup.

## Ostatnie akcje
1. IMP2-2.2 zmergowane (#1519, `5948dd1c`), #1478 zamknięte z live-stack HTTP proof. Adversarial review 28 agentów (7/24): 2 stream-leak + purge batch + basename defence naprawione przed merge.
2. IMP2-2.1 zmergowane (#1518, `1ee88ae8`), #1477 zamknięte (UTF-8-boundary + suffix-collision fixed).
3. Operator: ultracode ON; kontynuować etap 2 BEZ przeglądu zakresu.

## Następny krok — IMP2-2.3 (#1479)
Realna pauza/wznowienie/anulowanie + checkpoint crash-resilient (7–9h). `ImportRunHandler`: surowy SQL status-check per chunk (wzorzec `ExportJobHandler`) → paused/cancelled graceful stop + checkpoint; kolumny `checkpoint_offset`/`checkpoint_phase` na `ImportSession` (+migracja); resume re-dispatch od `checkpoint_offset+1` (reader przewija) z idempotentnymi licznikami; redelivery-aware (status-check na starcie); lock `refresh()` co chunk; state-machine `ensureTransitionable` (paused→running, running→cancelled bez LogicException); FE przywraca przyciski (flaga IMP2-0.3→true) + Playwright. Koordynacja: 1.8 two-pass (checkpoint_phase pass1/pass2) + 2.4 undo (first-write-wins). Najpierw understand-workflow (mapowanie ImportRunHandler + ExportJobHandler + ImportSessionStatus + deferred resume z 1.8/1.9).
