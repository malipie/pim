...
**Decyzja (ADR-014, Option Y — MODRC-01..04, 2026-05-26):** Atrybut typu `relation` to **zwykły atrybut**. Placement na karcie obiektu (zakładka vs. sekcja inline) wynika wyłącznie z `AttributeGroup` + jej `display_mode` na junction `object_type_attribute_groups` (kolumna `display_mode VARCHAR(8) NOT NULL DEFAULT 'tab' CHECK IN ('tab','stacked')` — MODR-01 #923). **Widget dobierany jest po typie atrybutu** (`relation` → picker + grid + preview card; `text` → input), ale samo umiejscowienie idzie po grupie. Brak hardcoded ścieżki dla typu `relation`. **Żadna grupa nie jest seedowana ani built-in dla relacji** — operator świadomie tworzy grupę i wybiera nazwę (MODRC-02 #1068 zapewnia inline create-group w konfiguratorze atrybutu).

**Konfigurator atrybutu `relation`** (MODRC-02 #1068) **nie pre-selectuje żadnej grupy**. Każdy atrybut musi mieć grupę (zapis bez grupy → 422). Inline akcja „+ Utwórz nową grupę" w selektorze pozwala stworzyć grupę i podpiąć atrybut w jednym przepływie. Ten sam pattern stosuje się do każdego typu atrybutu — brak typu z magicznym domyślnym przypisaniem do grupy.

**Sekcja systemowa „Powiązania zwrotne"** (MODRC-03 #1069): jedyna *wirtualna* zakładka w systemie, renderuje się automatycznie gdy `GET /api/objects/{id}/relations/reverse/count` zwróci `hasReverse=true`. Read-only, wizualnie odróżnialna od user-defined zakładek (ikona „system" / dedykowana stylistyka). Reverse to jedyny case, którego user nie zaprojektuje z góry — inni operatorzy mogą do tego obiektu wskazywać, więc system zapewnia minimalny widok bez zmuszania do utworzenia grupy „na zapas". Brak zakładki = brak reverse linków. Edycja po stronie źródła.
...
- ❌ **Reguła „relacja zawsze zakładka"** — odrzucona przez MODR-01..03 (Opcja 2). Placement to wyłącznie `display_mode` na junction; relacja zachowuje się jak każdy atrybut, widget dobierany po typie.
- ❌ **Seed grupy „Powiązania" jako built-in** — odrzucony przez MODRC-01 (#1067, Option Y, 2026-05-26). Anti-pattern discoverability: user nie miał jak odkryć, że dodanie atrybutu typu `relation` zmaterializuje zakładkę. Każda grupa musi być świadomie utworzona przez operatora (MODRC-02 inline create-group flow). Warunek powrotu: gdy onboarding ujawni, że >50% nowych tenantów potrzebuje grupy „Powiązania" w pierwszym tygodniu — onboarding wizard może *zaproponować* utworzenie tej grupy, ale nigdy nie zrobi tego implicite za operatora.
- ❌ **Flaga `has_relations` na ObjectType** — odrzucona przez MODRC-01..03 (Option Y, 2026-05-26). Proliferacja flag (`has_pricing`, `has_translations`, `has_seo`…) prowadzi do Pimcore-classes anti-patternu. Capability dla zakładki rozstrzygnięta przez obecność user-defined grupy + ewentualną sekcję systemową „Powiązania zwrotne".
- ❌ **„Magiczna" widoczność zakładki przy populacji atrybutami** — odrzucona przez MODRC-01..03 (Option Y, 2026-05-26). User nie ma jak odkryć tej reguły. Zakładka istnieje wtedy i tylko wtedy, gdy operator świadomie utworzył grupę z `display_mode='tab'` (lub gdy obiekt jest targetem reverse relation — wówczas systemowa sekcja jest jawnie wirtualna i wizualnie odróżnialna).
